...
export const WrittenExpressionSection: React.FC<WrittenExpressionSectionProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h5 className="text-[9px] sm:text-[10px] font-black uppercase text-indigo-400 dark:text-indigo-300 mb-3 tracking-widest">
          {t('results.yourWriting')}
        </h5>
        <div className="bg-white dark:bg-slate-800 rounded-xl sm:rounded-2xl p-4 sm:p-6 border-2 border-slate-300 dark:border-slate-600">
      ...
      {/* Corrections */}
      <div>
        <h5 className="text-[9px] sm:text-[10px] font-black uppercase text-amber-600 dark:text-amber-400 mb-3 tracking-widest">
          {t('results.corrections')}
        </h5>
        {corrections.length > 0 ? (
          <div className="space-y-3 sm:space-y-4">
            {corrections.map((sentence: UpgradedSentence, i: number) => (
              <div key={i} className="bg-amber-50 dark:bg-amber-900/20 p-4 sm:p-5 rounded-xl border border-amber-200 dark:border-amber-800">
                <div className="grid md:grid-cols-2 gap-3 sm:gap-4 mb-3">
                  <div>
                    <div className="text-[9px] sm:text-[10px] font-black uppercase text-rose-600 dark:text-rose-400 mb-2 tracking-wider">
                      {t('results.original')}
                    </div>
                  ...
                  <div>
                    <div className="text-[9px] sm:text-[10px] font-black uppercase text-emerald-400 dark:text-emerald-300 mb-2 tracking-wider">
                      {t('results.corrected')}
                    </div>
          ...
          <div className="bg-amber-50/50 dark:bg-amber-900/10 p-4 rounded-xl border border-amber-200/50 dark:border-amber-800/50">
      ...
      {/* Model Answer */}
      <div>
        <h5 className="text-[9px] sm:text-[10px] font-black uppercase text-emerald-400 dark:text-emerald-300 mb-3 tracking-widest">
          {t('results.modelAnswer')}
        </h5>
        {hasModelAnswer ? (
          <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-emerald-200 dark:border-emerald-800">
