...
export function D2CReadingTab() {
  ...
  const fetchTaskList = useCallback(async () => {
    try {
      ...
      const data = await authenticatedFetchJSON<TaskListResponse>(
        `${BACKEND_URL}/api/tasks/active/reading`,
        { method: 'GET', getToken }
      );
      setTaskList(data.tasks);
    } catch (err) {
      console.error('Failed to fetch reading task list:', err);
  ...
  const handleStart = async (task: TaskWithAttempt) => {
    ...
    try {
      ...
      setTaskData({ task: data.task, questions: data.questions });
  ...
  const handleComplete = (result: MCQResult) => {
    if (selectedTask && sessionId) {
      localStorage.removeItem(`reading_exam_${selectedTask.taskId}_${sessionId}`);
    }
    setSelectedTask(null);
    setTaskData(null);
    setSessionId(null);
