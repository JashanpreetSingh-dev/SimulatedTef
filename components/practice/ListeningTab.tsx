...
export function ListeningTab() {
  ...
  const loadAssignments = async () => {
    try {
      ...
      if (isProfessor) {
        ...
        const published = await fetchPublishedAssignments('listening');
        if (published) {
        ...
        if (batchData) {
          ...
          const listeningAssignments = assigned
            .map((ba: any) => ba.assignment)
            .filter((a: any) => a && a.type === 'listening');
          setAssignments(listeningAssignments);
        } else {
          setAssignments([]);
        }
  ...
  const handleStartAssignment = (assignment: Assignment) => {
    ...
    navigate(`/exam/listening?taskId=${assignment.taskId}&assignmentId=${assignment.assignmentId}`);
  };
      ...
      <div className="space-y-4 md:space-y-6">
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="bg-gradient-to-br from-green-500 to-emerald-500 rounded-xl md:rounded-2xl p-3 md:p-4 shadow-lg"
            >
    ...
    return (
      <div className="text-center py-12">
        <p className="text-slate-500 dark:text-slate-400">
          {isProfessor 
            ? t('practice.noListeningAssignments')
            : batchName
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        {assignments.map((assignment) => (
          ...
          >
            ...
            <p className="text-green-100 text-xs leading-relaxed mb-2 md:mb-3 line-clamp-2">
              {assignment.prompt}
            </p>
