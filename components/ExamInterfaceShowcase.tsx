      ...
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-8 sm:mb-10">
          <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black text-white mb-4 sm:mb-6 leading-[1.1] tracking-[-0.02em]">
            Real-time conversation <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 via-indigo-300 to-cyan-400">adhering to TEF Canada standards</span>
          </h2>
        ...
        <div className="space-y-3 md:space-y-6">
          {/* Demo Notice */}
          <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl px-4 py-3 flex items-center justify-center gap-2">
            <span className="text-amber-400 text-sm">ℹ️</span>
          ...
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="flex gap-2">
              <div className={`px-3 md:px-5 py-1.5 md:py-2.5 rounded-xl md:rounded-[1.25rem] text-[9px] md:text-[10px] font-black uppercase tracking-widest transition-all border ${
                currentPart === 'A' ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg' : 'bg-white dark:bg-slate-900 text-slate-400 border-slate-200 dark:border-slate-800'
              }`}>
                Partie A
              </div>
              <div className={`px-3 md:px-5 py-1.5 md:py-2.5 rounded-xl md:rounded-[1.25rem] text-[9px] md:text-[10px] font-black uppercase tracking-widest transition-all border ${
                currentPart === 'B' ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg' : 'bg-white dark:bg-slate-900 text-slate-400 border-slate-200 dark:border-slate-800'
              }`}>
                Partie B
              </div>
            </div>
            <div className="flex items-center gap-2 md:gap-4">
              ...
              {status === 'active' && (
                <div className="text-[9px] md:text-[10px] font-black text-rose-500 flex items-center gap-1">
                  <span className="w-1.5 h-1.5 md:w-2 md:h-2 bg-rose-500 rounded-full animate-pulse shadow-[0_0_8px_rgba(244,63,94,0.6)]" /> 
                  LIVE
                </div>
              )}
            </div>
          ...
          <div className="grid lg:grid-cols-2 gap-4 md:gap-8">
            {/* Task Document Card */}
            <div className="bg-white dark:bg-slate-900 rounded-2xl md:rounded-[2.5rem] border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm flex flex-col h-[300px] md:h-[400px] transition-colors">
              <div className="bg-slate-900 dark:bg-slate-800 px-4 md:px-8 py-3 md:py-4 flex items-center justify-between border-b border-white/5">
                <span className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Document #1</span>
                <button className="text-white hover:text-indigo-400 transition-colors text-xs font-bold flex items-center gap-1">
              ...
              <div className="flex-1 overflow-y-auto p-4 md:p-6 bg-slate-50 dark:bg-slate-900/50 relative group scrollbar-hide">
                <div className="bg-slate-800/30 rounded-xl md:rounded-2xl p-8 border-2 border-dashed border-slate-700/50 flex items-center justify-center min-h-[200px] mb-4 md:mb-8">
                  <div className="text-center">
                    <div className="text-4xl mb-2">📋</div>
                    <p className="text-slate-400 text-sm">Document Officiel</p>
                    <p className="text-slate-500 text-xs mt-1">(Preview de l'interface d'examen)</p>
                  </div>
                </div>
                <div className="mt-4 md:mt-8 p-4 md:p-6 bg-white dark:bg-slate-800/80 backdrop-blur rounded-xl md:rounded-[2rem] border border-slate-100 dark:border-slate-700 text-[10px] md:text-xs leading-relaxed text-slate-600 dark:text-slate-300 italic shadow-sm">
                  <strong className="text-slate-900 dark:text-white not-italic block mb-1">Consigne :</strong> 
            ...
            {/* Mic Section */}
            <div className="bg-slate-900 dark:bg-slate-900 rounded-2xl md:rounded-[2.5rem] p-4 md:p-8 flex flex-row md:flex-col items-center justify-between md:justify-center gap-4 md:gap-0 md:space-y-8 relative overflow-hidden shadow-2xl transition-colors">
              <div className={`absolute inset-0 opacity-10 pointer-events-none transition-all duration-1000 ${
                isModelSpeaking ? 'bg-indigo-500' : (isUserSpeaking ? 'bg-emerald-500' : 'bg-transparent')
              }`} />
              
              <div className="relative group flex-shrink-0">
                <div className={`absolute inset-0 rounded-full blur-[60px] transition-all duration-700 ${
                  isModelSpeaking ? 'bg-indigo-500/40 scale-150' : (isUserSpeaking ? 'bg-emerald-500/40 scale-125' : 'bg-white/5 scale-100')
                }`} />
                <button
              ...
              <div className="text-left md:text-center space-y-1 md:space-y-2 z-10 flex-1 md:flex-none">
                <h4 className="text-white font-black text-sm md:text-xl tracking-tight">
                  {status === 'active' ? (isModelSpeaking ? 'L\'examinateur répond...' : 'À vous de parler') : 'Prêt pour l\'épreuve ?'}
                </h4>
                <p className="text-slate-400 text-[7px] md:text-[9px] uppercase font-black tracking-[0.4em]">TEF AI Master Simulator</p>
              </div>
            </div>
          </div>
        </div>
