 ...
 * Displays previous/next buttons and submit button
 */
    ...
    <div className="mb-4">
      ...
      `}>
        <div className="flex flex-row gap-2">
          <button
            onClick={onPrevious}
            disabled={isFirstQuestion || (!isPracticeAssignment && phase !== 'answering')}
            className={`
              flex-1 py-2 px-3 rounded-lg text-sm font-semibold transition-colors flex items-center justify-center gap-1
              ${isFirstQuestion || (!isPracticeAssignment && phase !== 'answering')
          ...
          <button
            onClick={onNext}
            disabled={isLastQuestion || (!isPracticeAssignment && phase !== 'answering')}
            className={`
              flex-1 py-2 px-3 rounded-lg text-sm font-semibold transition-colors flex items-center justify-center gap-1
              ${isLastQuestion || (!isPracticeAssignment && phase !== 'answering')
                ? 'bg-slate-300 text-slate-500 cursor-not-allowed'
                : theme === 'dark'
                ? 'bg-slate-700 hover:bg-slate-600 text-slate-100'
                : 'bg-slate-200 hover:bg-slate-300 text-slate-700'
