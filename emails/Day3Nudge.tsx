...
export const Day3NudgeEmail = ({
  ...
  return (
    <Html lang="fr">
      <Head>
        ...
        <style>{`
          @media only screen and (max-width: 620px) {
            ...
            .email-header, .email-hero, .email-cta, .email-footer {
      ...
      <Preview>Votre première session TEF vous attend — 15 minutes suffisent</Preview>
      <Body style={body}>
        <Container style={card} className="email-card">
          ...
          <Section style={heroSection} className="email-hero">
            <Heading as="h1" style={heroTitle} className="email-hero-title">
              {firstName}, vous n'avez pas encore commencé 👋
            </Heading>
            <Text style={heroText}>
              Vous vous êtes inscrit(e) il y a 3 jours. Votre examen TEF Canada approche — chaque
              session de pratique compte.
            </Text>
            <Text style={heroText}>
              Votre première session prend{' '}
              <strong>moins de 15 minutes</strong>. L'IA vous donnera un feedback immédiat sur votre
              expression orale.
            </Text>
          </Section>

          <Section style={ctaSection} className="email-cta">
            <Button href={practiceUrl} style={primaryButton}>
          ...
          <Section style={tipsSection}>
            ...
            <Row>
              <Column style={tipPill}>
                <Text style={tipText}>📊 <strong>Feedback</strong> — Score CLB, grammaire, lexique</Text>
              </Column>
            </Row>
          </Section>
...
export default Day3NudgeEmail;

Day3NudgeEmail.PreviewProps = {
  firstName: 'Jean',
  practiceUrl: 'http://localhost:3000/practice',
