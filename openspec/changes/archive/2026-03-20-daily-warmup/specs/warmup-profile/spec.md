...
### Requirement: Level estimate is shown to the user
The system SHALL display the user's current levelEstimate (e.g. "A2", "B1", "B2") on the warm-up dashboard and coach card.

#### Scenario: Level shown on dashboard
- **WHEN** the user visits `/warmup`
- **THEN** the system SHALL display their current levelEstimate from warmupUserProfile (or "A2" if no profile exists yet)

#### Scenario: Level shown on coach card
- **WHEN** the completion screen is displayed
- **THEN** the system SHALL show the levelAtSession value returned by `/session/complete`, alongside the levelNote from feedback
