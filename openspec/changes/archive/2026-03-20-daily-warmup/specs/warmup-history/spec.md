...
The system SHALL provide `GET /api/warmup/history` returning the last 30 session records for the authenticated user, suitable for a calendar dot view on the dashboard.
...
The system SHALL display the last 7 days as dot indicators on the warm-up dashboard, showing which days had a completed session.

#### Scenario: Completed day shown as filled dot
- **WHEN** a warmupSession with status "completed" exists for a given date
- **THEN** the corresponding calendar dot SHALL be filled/highlighted

#### Scenario: Missed day shown as empty dot
- **WHEN** no warmupSession exists for a given date within the last 7 days
- **THEN** the corresponding calendar dot SHALL be empty/unfilled
