...
export function ListeningPracticeView() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const data = await authenticatedFetchJSON<ListeningTask[]>(
          `${BACKEND_URL}/api/tasks/all?type=listening`,
          { getToken: async () => getToken() }
        );
        setTasks(data);
      } catch (err: any) {
        setError(err.message || t('practice.failedToLoadListening'));
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <DashboardLayout>
      <main className="max-w-5xl mx-auto p-4 md:p-8">
        ...
        <div className="mb-6">
          <PracticeTabNavigation activeTab={activeTab} onTabChange={setActiveTab} module="listening" />
        </div>

        {activeTab === 'history' && (
          <HistoryList module="listening" />
        )}
        ...
        {activeTab === 'practice' && !loading && !error && tasks.length === 0 && (
          <div className="text-slate-500 dark:text-slate-400 text-center py-16">
            {t('practice.noListeningTestsFree')}
          </div>
        )}
        ...
        {activeTab === 'practice' && !loading && !error && tasks.length > 0 && (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {tasks.map((task, index) => (
              <div
                key={task.taskId}
                onClick={() => navigate(`/practice/listening/${task.taskId}`)}
                className="bg-gradient-to-br from-green-500 to-emerald-500 rounded-2xl p-5 shadow-lg hover:shadow-xl hover:shadow-green-500/20 transition-all group cursor-pointer"
              >
                ...
                <h3 className="text-white font-bold text-base mb-1 line-clamp-2">
                  {t('practice.listeningTestTitle', { number: String(index + 1) })}
                </h3>
                <p className="text-green-100 text-xs leading-relaxed mb-4 line-clamp-2">
                  {task.prompt}
                </p>
                <div className="flex items-center justify-between">
                  <span className="text-green-200 text-xs font-medium">{t('practice.listeningTestMeta')}</span>
